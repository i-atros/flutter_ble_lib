...
class DeviceDetailsBloc {
  ...
  Stream<Null> get disconnectedDevice => _deviceRepository.pickedDevice.skipWhile((bleDevice) => bleDevice != null).cast<Null>();

  List<DebugLog> _logs = [];
  Logger log;
  Logger logError;

  DeviceDetailsBloc({DeviceRepository deviceRepository, BleManager bleManager})
  ...
  void readRssi() {
    _clearLogs();
    PeripheralTestOperations(_bleManager, _bleDevice.peripheral, log, logError).testReadingRssi();
  }

  void discovery() {
    _clearLogs();
    PeripheralTestOperations(_bleManager, _bleDevice.peripheral, log, logError).discovery();
  }

  void fetchConnectedDevices() {
    _clearLogs();
    PeripheralTestOperations(_bleManager, _bleDevice.peripheral, log, logError).fetchConnectedDevice();
  }

  void fetchKnownDevices() {
    _clearLogs();
    PeripheralTestOperations(_bleManager, _bleDevice.peripheral, log, logError).fetchKnownDevice();
  }

  void readCharacteristicForPeripheral() {
    _clearLogs();
    PeripheralTestOperations(_bleManager, _bleDevice.peripheral, log, logError).readCharacteristicForPeripheral();
  }

  void readCharacteristicForService() {
    _clearLogs();
    PeripheralTestOperations(_bleManager, _bleDevice.peripheral, log, logError).readCharacteristicForService();
  }
  ...
  void writeCharacteristicForPeripheral() {
    _clearLogs();
    PeripheralTestOperations(_bleManager, _bleDevice.peripheral, log, logError).writeCharacteristicForPeripheral();
  }

  void writeCharacteristicForService() {
    _clearLogs();
    PeripheralTestOperations(_bleManager, _bleDevice.peripheral, log, logError).writeCharacteristicForService();
  }

  void writeCharacteristicDirectly() {
    _clearLogs();
    PeripheralTestOperations(_bleManager, _bleDevice.peripheral, log, logError).writeCharacteristic();
  }

  void monitorCharacteristicForPeripheral() {
    _clearLogs();
    PeripheralTestOperations(_bleManager, _bleDevice.peripheral, log, logError).monitorCharacteristicForPeripheral();
  }

  void monitorCharacteristicForService() {
    _clearLogs();
    PeripheralTestOperations(_bleManager, _bleDevice.peripheral, log, logError).monitorCharacteristicForService();
  }

  void monitorCharacteristicDirectly() {
    _clearLogs();
    PeripheralTestOperations(_bleManager, _bleDevice.peripheral, log, logError).monitorCharacteristic();
  }

  void disableBluetooth() {
    _clearLogs();
    PeripheralTestOperations(_bleManager, _bleDevice.peripheral, log, logError).disableBluetooth();
  }

  void enableBluetooth() {
    _clearLogs();
    PeripheralTestOperations(_bleManager, _bleDevice.peripheral, log, logError).enableBluetooth();
  }

  void fetchBluetoothState() {
    _clearLogs();
    PeripheralTestOperations(_bleManager, _bleDevice.peripheral, log, logError).fetchBluetoothState();
  }
